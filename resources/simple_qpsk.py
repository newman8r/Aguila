#!/usr/bin/env python3
"""
Simple QPSK Transmitter for Aguila
Uses HackRF to transmit binary data using Quadrature Phase Shift Keying
Stripped down to basics to ensure reliable operation
"""
...
class SimpleQPSKTransmitter(gr.top_block):
    def __init__(self, freq_mhz=433, baud_rate=2000, gain=40, sample_rate=2000000, message="Hello from QPSK!"):
        gr.top_block.__init__(self, "Simple QPSK Transmitter")
    ...
    def setup_blocks(self):
        print(f"Initializing Simple QPSK transmitter with parameters:")
        ...
        self.chunks_to_symbols = digital.chunks_to_symbols_bc(
            [0.707+0.707j, -0.707+0.707j, 0.707-0.707j, -0.707-0.707j],  # QPSK constellation
            2  # 2 bits per symbol
        )
        
        # 4. Interpolate to match sample rate using repeat
        self.repeat = blocks.repeat(gr.sizeof_gr_complex, self.samples_per_symbol)
        
        # 5. Low-pass filter to smooth the signal
        cutoff_freq = self.baud_rate * 1.1  # Slightly wider than symbol rate
        trans_width = self.baud_rate * 0.5
        
        # Create a low-pass filter with default window type
        self.lpf = filter.firdes.low_pass(
            1.0,                  # Gain
        ...
        self.filter = filter.fir_filter_ccf(1, self.lpf)
        ...
        self.connect(self.source, self.unpacker, self.chunks_to_symbols, 
                    self.repeat, self.filter, self.hackrf_sink)
    ...
    def create_data_pattern(self):
        ...
        # Create a simple preamble for synchronization
        preamble = bytes([0xAA, 0xAA, 0xAA, 0xAA])  # 10101010 repeated
        
        # Create postamble
        postamble = bytes([0xAA, 0xAA, 0xAA, 0xAA])
        ...
        data_packet = preamble + length_byte + message_bytes + postamble
        
        # Add some padding zeros between repetitions
        padding = bytes([0x00] * 8)
        
        # Repeat the packet multiple times with padding
        full_data = data_packet + padding
        full_data = full_data * 10  # Repeat 10 times
        
        print(f"  - Packet structure: Preamble + Length + Message + Postamble")
...
def main():
    parser = argparse.ArgumentParser(description='Simple QPSK Transmitter')
